    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($path === '/admin/settings') {
            try {
                ...
                $values=['smtp_host'=>$host,'smtp_port'=>$port,'smtp_security'=>$security,'smtp_username'=>$username,'smtp_from'=>$from];
...
<?php endif ?></section>
<?php else: ?>
<h1>Website beheren</h1><nav class="admin-tabs" aria-label="Beheer"><a href="/admin" <?= $path === '/admin' ? 'aria-current="page"' : '' ?>>Pagina’s</a><a href="/admin/messages" <?= $path === '/admin/messages' ? 'aria-current="page"' : '' ?>>Contactaanvragen</a><a href="/admin/settings" <?= $path === '/admin/settings' ? 'aria-current="page"' : '' ?>>E-mailinstellingen</a><a href="/admin/password" <?= $path === '/admin/password' ? 'aria-current="page"' : '' ?>>Wachtwoord</a></nav>
<?php if ($path === '/admin/settings'): $c=config(); ?>
<section class="panel admin-panel"><h2>E-mail versturen</h2><p>Alle contactaanvragen worden doorgestuurd naar <strong><?= e(CONTACT_RECIPIENT) ?></strong>. Gebruik de SMTP-gegevens van je mailprovider. Je wachtwoord wordt nooit in dit formulier teruggetoond.</p><p class="notice <?= smtp_configured()?'success':'error' ?>"><?= smtp_configured()?'SMTP-gegevens zijn ingevuld. Aflevering moet nog met een echte aanvraag worden gecontroleerd.':'SMTP is nog niet ingesteld. Contactaanvragen worden wel opgeslagen, maar nog niet per e-mail doorgestuurd.' ?></p>
<form method="post"><input type="hidden" name="csrf" value="<?= e(csrf()) ?>"><div class="editor-fields"><label>SMTP-server<input name="smtp_host" required value="<?= e($c['smtp_host']) ?>" placeholder="Bijvoorbeeld smtp.jouwprovider.nl"></label><label>Beveiliging<select name="smtp_security"><option value="tls" <?= $c['smtp_security']==='tls'?'selected':'' ?>>STARTTLS (poort 587)</option><option value="ssl" <?= $c['smtp_security']==='ssl'?'selected':'' ?>>SSL/TLS (poort 465)</option></select></label><label>Poort<input name="smtp_port" type="number" required value="<?= e($c['smtp_port']) ?>"></label><label>Gebruikersnaam<input name="smtp_username" required autocomplete="off" value="<?= e($c['smtp_username']) ?>"></label><label>SMTP-wachtwoord<input name="smtp_password" type="password" autocomplete="new-password" placeholder="Laat leeg om het bestaande wachtwoord te behouden"></label><label>Afzenderadres<input name="smtp_from" type="email" required value="<?= e($c['smtp_from']) ?>"></label></div><button class="button">Mailinstellingen opslaan</button></form></section>
